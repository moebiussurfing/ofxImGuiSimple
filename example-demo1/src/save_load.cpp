...
//            if (num_selected > 0 && ImGui::IsKeyReleased(SDL_SCANCODE_X))
            if (num_selected > 0 && ImGui::IsKeyReleased(1))
                {
...
//            std::cout << s << '\n';
            ...
            for (const auto& link : links_)
            {
                ss << "[link]\n";
                ss << "id=" << link.first << "\n";
                ss << "data=" << link.second.start << "," << link.second.end
                << "\n\n";
            }
            std::string s = ss.str();
            std::cout << s << '\n';
    ...
    void save()
    {
        ...
        std::string str = ofToDataPath("imnodes.ini");
        const char * c = str.c_str();

//        imnodes::SaveCurrentEditorStateToIniFile("imnodes.ini");
        imnodes::SaveCurrentEditorStateToIniFile(c);
    }
